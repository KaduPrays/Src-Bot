...
module.exports = async function TERCEIRO(user, message, emoji3, bot) {
    
    const Discord = require("discord.js");
    const colors = require("../../colors.json");
    const config = require("../../config.json");

    const nome = `${emoji3}・${user.username}`
    ...
    message.guild.channels.create(nome, {
        ...
        channel.send({
            embed: {
                title: `▫️ Aberto dia ${moment(new Date()).tz("America/Sao_Paulo").format("LLL")}`,
                        ...
                        channel.send({
                            ...
                            channel.send({
                                ...
                                Excluir.on('collect', async r => {
                                    ...
                                    }).then(msg => { setTimeout(() => { channel.delete() }, 10 * 1000) });
                                })
                            })
                        })
                    break;

module.exports = async function PRIMEIRO(user, message, emoji1, bot) {
    ...
    const nome = `${emoji1}・${user.username}`
                            ...
                            channel.send({
                                ...
                                Excluir.on('collect', async r => {
                                    ...
                                    const embed = new Discord.MessageEmbed()
                                        ...
                                        .setAuthor('Registro de Suporte', bot.user.displayAvatarURL())
                                        .setDescription(`▫ **Staff:** ${r.users.cache.find(user => user.bot === false).tag}
                                        ▫ **Membro:** ${user.tag}
                                        ▫ **Data:** ${momente(new Date()).tz("America/Sao_Paulo").format('LLL')}
                                        ▫ **ID:** ${user.id}`)
                                    logs.send(embed)
                                    logs.send({ files: [`${aleatorio}.txt`] })
                                    setTimeout(function () {

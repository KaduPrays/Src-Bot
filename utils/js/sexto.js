...
module.exports = async function SEXTO(user, message, emoji6, bot) {
    
    const Discord = require("discord.js")
    const colors = require("../../colors.json");
    const config = require('../../config.json');

    const nome = `${emoji6}・${user.username}`
    ...
    message.guild.channels.create(nome, {
        ...
        channel.send({
            embed: {
                ...
                description: "Envie sua denúncia no chat, tentando explicá-la em detalhes.\nPara fechar esse canal de suporte basta clicar na reação abaixo:",
                color: colors.none
            }
        }).then(async msg => {
